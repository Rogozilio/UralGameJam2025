...
{
    public PlantGenerator plantGenerator; // Точки, которые мы создали ранее
    ...
    {
        if (plantGenerator.points == null || plantGenerator.points.Count < 2)
    {
    ...
    for (int i = 0; i < plantGenerator.points.Count; i++)
    {
        Vector3 forward = i < plantGenerator.points.Count - 1
            ? (plantGenerator.points[i + 1] - plantGenerator.points[i]).normalized
            : (plantGenerator.points[i] - plantGenerator.points[i - 1]).normalized;
    ...
    for (int i = 0; i < plantGenerator.points.Count; i++)
    {
        ...
        for (int j = 0; j < segments; j++)
        {
            ...
            vertices.Add(plantGenerator.points[i] + offset);

            // Добавляем UV
            uvs.Add(new Vector2((float)j / segments, (float)i / plantGenerator.points.Count));
        }
    }

    // Создаем треугольники
    for (int i = 0; i < plantGenerator.points.Count - 1; i++) // Для каждого сегмента между точками
    {
